...
void mdb_entry_free(MdbEntry *mdb_entry) {
        if (!mdb_entry)
                return;
...
static int set_mdb_handler(sd_netlink *rtnl, sd_netlink_message *m, Link *link) {
        ...
        if (r < 0 && r != -EEXIST) {
                log_link_error_errno(link, r, "Could not add MDB entry: %m");
...
int mdb_entry_configure(Link *link, MdbEntry *mdb_entry) {
        ...
        assert(link);
        assert(link->network);
        assert(link->network->bridge);
        assert(link->manager);
        assert(mdb_entry);

        entry.state = MDB_PERMANENT;
        entry.ifindex = link->ifindex;
        entry.vid = mdb_entry->vlan_id;
        ...
        case AF_INET6:
                memcpy(&entry.addr.u.ip6, &mdb_entry->group_addr.in6, sizeof(entry.addr.u.ip6));
                entry.addr.proto = htons(ETH_P_IPV6);
                break;

        default:
                return log_link_error(link, "Invalid network family");
        }
