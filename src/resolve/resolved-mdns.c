...
static int on_mdns_packet(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if (dns_packet_validate_reply(p) > 0) {
                 ...
                 *
                 * Hence, extract the packet here, and assemble a question that matches the answers
                 * we got. Then try to find a pending transaction that is waiting for that information,
                 * and complete it if we have any. Otherwise, just store the new information in the
                 * scope's cache.
                 */
